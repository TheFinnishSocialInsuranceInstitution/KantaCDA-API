...
public class ReseptinUusimisKasaaja extends ReseptiKasaaja {
    ...
    private void paivataUusimisTiedot() {
        ...
        if ( null == uusi.isAnnostusTarvittaessa()) {
        	uusi.setAnnostusTarvittaessa(alkuperainen.isAnnostusTarvittaessa());
        }
		if (null == uusi.getAnnosajaksonPituus()) {
			uusi.setAnnosajaksonPituus(alkuperainen.getAnnosajaksonPituus());
		}
		if (uusi.getAnnokset().isEmpty()) {
			uusi.getAnnokset().addAll(alkuperainen.getAnnokset());
		}
